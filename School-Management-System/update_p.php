...
if($check->num_rows==0)
        ...
        $getid=$mysqli->query("select pr_id from project where sid=$sid and cid='$cid'");
          if($getid->num_rows==0)
           {   
        ...
        $res=$getid->fetch_assoc();
        $pid=$res['pr_id'];
        $date=$_POST['submit-date'];
        echo $date;
        $marks=$_POST['marks'];
        $qu="update submit set submit_date='$date',marks=$marks where pid='$pid'";
...
<div class="dropdown btn-group" >
    ...
    <input type="number" id="sid" name="sid" placeholder="Enter ID.." style="width: 300px; height:40px;">
    <br>
    <br>
    
    <label for="submission date">Enter Submitted Date</label>
    <input type="date" id="submit date" name="submit-date" placeholder="Date.." style="width: 300px; height:40px;">
    <br>
    <br>
    
    <label for="lname">Enter Marks</label>
    <input type="number" min="0" max="20" id="marks" name="marks" placeholder="Enter Marks.." style="width: 300px; height:40px;">
