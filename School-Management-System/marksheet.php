...
<div class="dropdown btn-group" >
 ...
 <table id="<?=$type?>" style="width:75%;" align="center">
 <tr>
         <th>Subject</th>
         <th>Marks</th>
     </tr>
  <?php
      ...
      $q=$mysqli->query("SELECT pid from paper_i where sid=$sid and eid in(select eid from exam_i where exam='$type')");
      while($res=$q->fetch_assoc())
      { $pid=$res['pid'];
      $q2=$mysqli->query("SELECT sname from subject where subid in(select subid from exam_i where eid in(select eid from paper_i where pid='$pid'))");
      $res2=$q2->fetch_assoc();
      $cname=$res2['sname'];
      $q3=$mysqli->query("SELECT marks from marks_i where pid='$pid'");
      $res3=$q3->fetch_assoc();
      $marks=$res3['marks'];
      ?>
     <tr>
 ...
 <tr>
  ...
  <?php
      ...
      $q=$mysqli->query("SELECT pid from paper_e where sid=$sid and eid in(select eid from exam_e where exam='$type')");
      while($res=$q->fetch_assoc())
      { $pid=$res['pid'];
      $q2=$mysqli->query("SELECT sname from subject where subid in(select subid from exam_e where eid in(select eid from paper_e where pid='$pid'))");
      $res2=$q2->fetch_assoc();
      $cname=$res2['sname'];
      $q3=$mysqli->query("SELECT marks from marks_e where pid='$pid'");
      $res3=$q3->fetch_assoc();
      $marks=$res3['marks'];
      ?>
     <tr>
