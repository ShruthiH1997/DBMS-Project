...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    if (isset($_POST['update'])) { 
        $fid=$_SESSION['fid'];
     $query=$mysqli->query("select sid from student where std=(SELECT std from class_teacher where fid=$fid) and dv=(SELECT dv from class_teacher where fid=$fid)");
     while($result=$query->fetch_assoc())
     {
       ...
       $month=$_POST['month'];
       $att=$_POST[$id];
       $query3=$mysqli->query("select w_days from attendance where month='$month'");
       $chk=$query3->fetch_assoc();
       if($att>$chk['w_days'])
       {
            header( "location: uodate_a.php" );
       }
       $query2=$mysqli->query("update attendance set p_days=$att where month='$month' and sid='$id' ");
...
<div class="dropdown btn-group" >
    ...
    <form method="POST" align="center">
     <label for="STD">CHOOSE MONTH:</label>
       <select id="month" name="month"  placeholder="Select..." style="width: 300px; height:40px;">
        <?php
        $q=$mysqli->query("SELECT DISTINCT month from attendance");
         while ($c=$q->fetch_assoc())
         {
        ?>
      <option value="<?=$c['month']?>" style="width: 300px; height:40px;"><?=$c['month']?></option>
         <?php } ?>
    ...
    <br> 
     <br> 
     <p>ENTER ATTENDANCE FOR STUDENT:</p>
        <?php
        while($result=$q1->fetch_assoc())
           {
          
            ?>
        <label for="<?=$result['sid']?>"><?=$result['sid']?> <?=$result['sname']?> : </label>
        <input type="number" min="0" max="25" id="fname" name="<?=$result['sid']?>" style="width: 300px; height:40px;">
    <br>    
    <br> 
     <br> 
