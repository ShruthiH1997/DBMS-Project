...
<div class="dropdown btn-group" >
  ...
  $q=$mysqli->query("SELECT * from attendance where sid='$sid'");
 $st=$mysqli->query("SELECT sname from student where sid='$sid'");
 $stu=$st->fetch_assoc();
   /*print "<table> n";
      foreach($q as $user)
      {
 ...
 */ ?>
    <div class="row" style="padding: 25px; font-size: 250%; font-weight: 900; color:#1ab188; text-align: center; height: 100px;">
        ATTENDANCE RECORDS FOR <?= strtoupper($stu['sname'])?>
        
    </div>
    
    
  <table id="attendance-student" style="width:75%;" align="center">
     <tr>
    <th>Month</th>
    <th>Working Days</th> 
